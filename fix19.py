...
# 1. Добавляем ADMIN_ID
old_trial = 'TRIAL_DAYS = 7'
new_trial = 'TRIAL_DAYS = 7\nADMIN_ID = 210064232'
text = text.replace(old_trial, new_trial)

# 2. Добавляем обработку /admin перед /start
old_start = '        if user_text.strip() == "/start":'
new_start = '''        if user_text.strip() == "/admin" and uid == ADMIN_ID:
